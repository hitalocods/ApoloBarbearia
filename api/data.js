...
export default async function handler(req, res) {
    ...
    try {
        ...
        const [barbeiros, servicos, horariosRows, agendamentos, despesas] = await Promise.all([
            query`SELECT id, nome, especialidade, whatsapp, foto FROM barbeiros ORDER BY created_at ASC`,
            ...
            query`SELECT id, descricao as desc, categoria as cat, to_char(data, 'YYYY-MM-DD') as data, valor::float as valor FROM despesas ORDER BY data DESC`
        ]);
